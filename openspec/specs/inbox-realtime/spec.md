...
Define la capa de tiempo real del inbox de Notify: cómo los cambios en las
conversaciones (entrantes, estados de entrega, reorden, no leídos, estado de
ventana) se propagan a los clientes en near-real-time mediante Centrífugo, sin
acoplar la capa de dominio al transporte. Cubre el puerto de publicación
desacoplado, el modelo de canales multi-tenant del namespace `notify_inbox`, la
autorización de suscripción por membresía de organización mediante tokens JWT, la
suscripción del cliente con revalidación de SWR, y el polling de respaldo como
mecanismo de resiliencia ante pérdidas temporales de conexión.
