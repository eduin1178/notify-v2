...
Define la REST API de Notify: una API versionada servida por Hono dentro del proyecto Next, con contrato declarado vía `@hono/zod-openapi`, documentación OpenAPI 3.x autogenerada, cliente RPC tipado end-to-end, manejo uniforme de errores y multi-tenancy expresada en el path. Esta API es el transporte compartido para la web (Next) y la futura app móvil (Expo).
